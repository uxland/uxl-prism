...
const getOrientation = (orientation: string | number) => {
    ...
    return <ScreenOrientationState>{orientation: orientation, type: orientation === '0' ? 'portrait' : 'landscape'};
};

const setOrientationActionCreator = createAction<ScreenOrientationState>(SET_ORIENTATION_ACTION);

const setAppOrientation = (dispatch: Dispatch<Action>) => dispatch(setOrientationActionCreator(getOrientation(window.orientation)));
