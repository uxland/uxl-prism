...
export const reducer: (state: GeoLocationState, action : Action) => GeoLocationState = (state: GeoLocationState = {lastKnownPosition: null, registeredWatchers: {}}, action: Action) =>{
    switch (action.type){
        case SET_LAST_POSITION_ACTION:
            return {...state, lastKnownPosition: action.payload};
        case REGISTER_GEO_LOCATION_WATCHER:
            return {...state, registeredWatchers: {...state.registeredWatchers, [action.meta]: action.payload}};
        case UNREGISTER_GEO_LOCATION_WATCHER:
            let newWatchers = {...state.registeredWatchers};
