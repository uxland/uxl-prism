...
//Dont remove "main" const.
//Is a webpack dynamic import hack for moduleLoader.
const main = "main.ts";
const moduleLoader = (module, appsBaseRoute, postFn) =>
  module && module.localModule
    ? import(/* webpackChunkName: "localModule" */ `src/${main}`).then(m =>
        postFn(module, m)
      )
    : import(/* webpackChunkName: "[request]" */ `@uxland-admin/${
        module.folder
      }/src/main.ts`).then(m => postFn(module, m));
...
class Bootstrapper extends propertiesObserver(Object) implements IBootstrapper {
  ...
  private async handleModulesChanged(
    modules: ModuleInfo[],
